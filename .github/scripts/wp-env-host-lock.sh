...
# runner host (one Raspberry Pi, one runner service per repo, all able to
# execute concurrently). GitHub's `concurrency:` key only serializes jobs
# within a single repository, so a different repo's wp-env can still start
# while this job's wp-env is up -- colliding on fixed host ports or starving
# the host's RAM. A flock on a fixed host-wide path closes that gap for every
# repo that calls this script with the same lockfile.
#
