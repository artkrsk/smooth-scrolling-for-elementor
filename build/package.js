...
export async function buildRelease(ctx) {
  ...
  // Empty staging IN PLACE — never delete the dir itself: a wp-env harness (in
  // template siblings; this repo has none) bind-mounts it into test containers,
  // and removing the mounted inode severs the mount (on Linux and macOS Docker
  // Desktop alike) until the environment restarts.
  ...
  // in one PHP process — which a PHPUnit harness (template siblings) does per run.
  ...
  stagedComposer.config = {
    ...stagedComposer.config,
    'autoloader-suffix': 'ArtsSmoothScrollingPlugin'
  }
  ...
  await buildJs(ctx, {
    dev: false,
    outfile: path.join(libraryOut, 'gate.js'),
...
function assertRelease(ctx) {
  ...
  const required = [
    ...
    `src/php/libraries/${slug}/gate.js`,
    `src/php/libraries/${slug}/${slug}.css`
  ]
