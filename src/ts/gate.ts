...
if (!window.artsSmoothScrolling) {
  ...
  if (!options || !boot) {
    ...
    const inject = () => {
      ...
      link.rel = 'stylesheet'
      link.href = boot.css
      // Chained on purpose: Lenis adds `.lenis` to <html> from its constructor
      // either way, but the class is inert without Lenis's own stylesheet —
      // that is what carries the `height: auto` rule the engine scrolls
      // against. A CSS 404 must therefore stop the engine from booting
      // instead of running it unstyled.
      link.onload = () => {
