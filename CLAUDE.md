...
- Compiled assets land in `src/php/libraries/smooth-scrolling-for-elementor/` and are **committed** — composer consumers get prebuilt assets. Never hand-edit `gate.js`, `smooth-scrolling-for-elementor.js/.css` there; edit `src/ts` / `src/styles` and rebuild.
...
- `tests/support.ts` holds shared factories (fake Lenis, controllable matchMedia); `tests/setup.ts` forces `import.meta.env.DEV` to false so suites run the shipped code path, not the dev diagnostics.
